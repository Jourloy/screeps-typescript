...
export class worker {
	...
	private static findForRepair(creep: Creep): boolean {
		...
		if (repairStructures.length > 0) {
			...
			if (creep.memory.busy?.id == null && repairStructures.length > 4) {
				const str = _.sample(repairStructures);
				creep.memory.busy!.id = str.id;
				creep.memory.busy!.type = str.structureType;
				creep.memory.busy!.aim = 'repair';
				return true;
			} else {
				return false;
	...
	private static findWork(creep: Creep) {
		...
		if (creep.room.controller!.ticksToDowngrade < 5000) {
			...
			const helpers = tools.amountCreeps(creep.room.name, 'helper');
			if (helpers === 0) {
				const notFullStructures = RoomCollector.info[creep.room.name].energyStore;
				if (notFullStructures.length > 0) {
					if (
						creep.transfer(notFullStructures[0], RESOURCE_ENERGY) === ERR_NOT_IN_RANGE
					) {
			...
			if (!this.findForBuild(creep)) {
				if (!this.findForRepair(creep) && creep.room.controller!.level > 2) {
					if (creep.upgradeController(creep.room.controller!) === ERR_NOT_IN_RANGE) {
						creep.travelTo(creep.room.controller!, { range: 3 });
					}
				}
	...
	private static isHelp(creep: Creep) {
		const transporters = tools.amountCreeps(creep.room.name, 'transporter');
		const miners = tools.amountCreeps(creep.room.name, 'miner');
		if (creep.room.controller!.level < 3) {
			const helpers = tools.amountCreeps(creep.room.name, 'helper');
			if ((transporters === 0 || miners === 0) && helpers === 0) {
				return true;
			}
		} else {
			if (transporters === 0 || miners === 0) {
				return true;
			}
		}
	...
	private static isNeed(creep: Creep): boolean {
		...
		if (creep.memory.busy?.aim === 'repair' && creep.room.controller!.level <= 2) {
			check = false;
		}
